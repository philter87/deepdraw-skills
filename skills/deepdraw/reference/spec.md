...
| `from`, `to` | arrows only |
| `points` | `draw` only |
| `href` | `image` (data URI) or `icon` (raw SVG markup) |
...
| `arrow` | — | `fill: none`, `arrowEnd: triangle` |
...
| `arrowEnd` | `triangle` | `none` makes a plain connector |
...
`href` on an `icon` node is raw SVG markup, rebuilt through an allowlist:
`svg g defs symbol use title desc path rect circle ellipse line polyline polygon
text tspan clipPath mask pattern linearGradient radialGradient stop`, with
presentation and geometry attributes. No `<image>`, no script, no animation, and
`href`/`url()` may only point inside the file.
